...
def main():
    ...
    kn = np.array([one_over_R, 0.0, 0.0])
    ks = np.array([0.0, 0.0, 0.0])

    cfb_obj1 = impactx.elements.ExactCFbend(ds=L,
                                            k_normal=kn,
                                            k_skew=ks,
                                            nslice=4,
                                            name='foo')

    print('obj1 contains: ', dir(cfb_obj1))
    print('obj1 dict: ', cfb_obj1.to_dict())
    ...
    B = pnorm * one_over_R
    print('B: ', B, 'T')
    knn = np.array([B, 0.0, 0.0])

    cfb_obj2 = impactx.elements.ExactCFbend(ds=L,
                                            k_normal=kn,
                                            k_skew=ks,
                                            unit=1,
                                            nslice=4,
    ...
    print('obj2 dict: ', cfb_obj2.to_dict())
