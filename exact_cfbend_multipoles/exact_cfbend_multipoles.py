...
# test k2 strength and whether it has factor of 2!
...
#cfb_knormal = np.array([1/mag_R, 0.0, 1.0e6])
#cfb_knormal = np.array([1/mag_R, 1.0, 0.0])
k1=0
cfb_knormal = np.array([k0, k1, k2])
cfb_kskew = np.array([0.0, 0.0, 0.0])
np.save('cfb_knormal.npy', cfb_knormal)
...
sim.lattice.extend(
    [
        ...
        # the kick from this multipole is approximately
        # -k_normal[1]*x - 0.5*k_normal[2] * x**2
        #elements.ExactMultipole(ds=mag_length, k_normal=cfb_knormal, k_skew=cfb_kskew, nslice=16, name="foo"),
