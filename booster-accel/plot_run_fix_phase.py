...
def main(saveplt):
    ...
    phase = np.array(runstatus['phase'])
    # fix phase since it wasn't saved with the transition correction
    for i in range(len(phase)):
        if runstatus['gamma'].iloc[i] >= gamma_tr:
            phase[i] = np.pi - phase[i]
 ...
 #    ax[1, 1].plot(np.select(runstatus['gamma'].to_numpy()>gamma_tr, np.pi-runstatus['phase'], runstatus['phase']), label='RF phase [rad]')
    ax[1, 1].plot(phase, label='RF phase [rad]')
    ax[1, 1].legend(loc='best')
    ax[1, 1].grid(True)
