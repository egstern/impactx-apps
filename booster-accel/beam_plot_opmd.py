...
def plot_density(x, y, label, options):
    ...
    fig.suptitle('ImpactX Phase Space Distribution')
...
def do_plots(options):
    c = 299792458.0
    series = io.Series(options.inputfile, io.Access_Type.read_only)
    ...
    part_df = beam.to_df()
    npart = len(part_df)
    particles = numpy.ndarray((npart, 11))

    #npart = particles.shape[0]
    mass = beam.get_attribute('mass_ref') * 1.0e-9 * c**2/eV
    #mass = h5.get('mass')[()]
    betagamma = beam.get_attribute('beta_gamma_ref')
    gamma = beam.get_attribute('gamma_ref')
    p_ref = mass * betagamma
    e_ref = mass * gamma
    beta = beam.get_attribute('beta_ref')
    print("p_ref: ", p_ref)
    print("mass: ", mass)

    particles[:, 0] = part_df['position_x']
    particles[:, 1] = part_df['momentum_x']
    particles[:, 2] = part_df['position_y']
    particles[:, 3] = part_df['momentum_y']
    particles[:, 4] = part_df['position_t']
    particles[:, 6] = part_df['momentum_t']
    particles[:, 5] = (numpy.sqrt( (-p_ref * particles[:, 6] + e_ref)**2 - mass**2 )-p_ref)/p_ref
    particles[:, 7] = numpy.sqrt( (-p_ref * particles[:, 6] + e_ref)**2 - mass**2 )
    particles[:, 8] = -p_ref * particles[:, 6] + e_ref
    particles[:, 9] = particles[:, 4]/c
    particles[:, 10] = particles[:, 4]*beta
